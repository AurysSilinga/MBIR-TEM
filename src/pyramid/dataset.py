...
from pyramid.phasemapper import PhaseMapperRDFC, PhaseMapperCharge
...
class DataSet(object):
    ...
    def plot_mask(self, pretty=False,**kwargs):
        ...
        if pretty:
            mask = np.pad(self.mask, 1, mode='constant')
        else:
            mask = self.mask
        
        if self.mask is not None:
            return ScalarData(self.a, mask).plot_mask(**kwargs)
...
class DataSetCharge(object):
    """Class for collecting phase maps and corresponding projectors.

    Represents a collection of (e.g. experimentally derived) phase maps, stored as
    :class:`~.PhaseMap` objects and corresponding projectors stored as :class:`~.Projector`
    objects. At creation, the grid spacing `a` and the dimension `dim` of the charge
    distribution have to be given. Data can be added via the :func:`~.append` method, where
    a :class:`~.PhaseMap`, a :class:`~.Projector` and additional info have to be given.
    ...
    _log = logging.getLogger(__name__ + '.DataSetCharge')
    ...
    @property
    def mask(self):
        """A boolean mask which defines the charged volume in 3D."""
        return self._mask
    ...
    @property
    def phasemaps(self):
        """List of all PhaseMaps in the DataSetCharge."""
        return self._phasemaps

    @property
    def projectors(self):
        """List of all Projectors in the DataSetCharge."""
        return self._projectors

    @property
    def phasemappers(self):
        # TODO: get rid, only use phasemapper_dict!!
        """List of all PhaseMappers in the DataSetCharge."""
        return self._phasemappers

    @property
    def phasemapper_dict(self):
        """Dictionary of all PhaseMappers in the DataSetCharge."""
        return self._phasemapper_dict

    def __init__(self, a, dim, electrode_vec, mask=None, Se_inv=None):
        dim = tuple(dim)
        self._log.debug('Calling __init__')
        assert isinstance(dim, tuple) and len(dim) == 3, \
            'Dimension has to be a tuple of length 3!'
    ...
    def create_phasemaps(self, elecdata, difference=False, ramp=None):
        """Create a list of phasemaps with the projectors in the dataset for a given
        :class:`~.VectorData` object.

        Parameters
        ----------
        elecdata : :class:`~.VectorData`
            Charge distribution to which the projectors of the dataset should be applied.
        difference : bool, optional
            If `True`, the phasemaps of the dataset are subtracted from the created ones to view
            difference images. Default is False.
        ramp : :class:`~.Ramp`
        ...
        phasemaps = []
        for i, projector in enumerate(self.projectors):
            c_proj = projector(elecdata)
            phasemap = self.phasemappers[i](c_proj)
            if difference:
                phasemap -= self.phasemaps[i]
            if ramp is not None:
                assert type(ramp) == Ramp, 'ramp has to be a Ramp object!'
                phasemap += ramp(index=i)  # Full formula: phasemap -= phasemap_dataset - ramp
            phasemap.mask = c_proj.get_mask()[0, ...]
            phasemaps.append(phasemap)
        return phasemaps
    ...
    def set_3d_mask(self, mask_list=None, threshold=0.9):
        ...
        threshold: float, optional
            The threshold, describing the minimal number of 2D masks which have to extrude to the
            point in 3D to be considered valid as containing charge. `threshold` is a
    ...
    def plot_mask(self):
        """If it exists, display the 3D mask of the charge distribution.
    ...
    def plot_phasemaps(self, elecdata=None, title='Phase Map', difference=False, ramp=None,
                       **kwargs):
        """Display all phasemaps saved in the :class:`~.DataSet` as a colormesh.

        Parameters
        ----------
        elecdata : :class:`~.VectorData`, optional
            Magnetic distribution to which the projectors of the dataset should be applied. If not
            given, the phasemaps in the dataset are used.
        title : string, optional
            The main part of the title of the plots. The default is 'Phase Map'. Additional
        ...
        """
        self._log.debug('Calling plot_phasemaps')
        if elecdata is not None:  # Plot phasemaps of the given charge distribution:
            phasemaps = self.create_phasemaps(elecdata, difference=difference, ramp=ramp)
        else:  # Plot phasemaps saved in the DataSet (default):
            phasemaps = self.phasemaps
            if ramp is not None:
                for i, phasemap in enumerate(phasemaps):
    ...
    def plot_phasemaps_combined(self, elecdata=None, title='Combined Plot', difference=False,
                                ramp=None, **kwargs):
        """Display all phasemaps and the resulting color coded holography images.
