...
SCALE = 1  # .0E-9 / 1.0E-2  # From cm to nm
filename = PATH+'magnetic_distribution_ring.txt'
data = np.genfromtxt(filename, skip_header=2)
dim = tuple(np.genfromtxt(filename, dtype=int, skip_header=1, skip_footer=len(data[:, 0])))
a = (data[1, 0] - data[0, 0]) / SCALE
magnitude = data[:, 3:6].T.reshape((3,)+dim)
x, y, z, zmag, ymag, xmag = data.T
#magnitude = np.asarray([xmag, ymag, zmag]).reshape((3,)+dim)
mag_data_cyl = MagData(a, magnitude)
mag_data_cyl.quiver_plot(proj_axis='x')
mag_data_cyl.quiver_plot3d()
phase_map_cyl = pm(mag_data_cyl, axis='x')
phase_map_cyl.display_combined()
